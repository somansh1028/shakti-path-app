...
const CareerGuideChat: React.FC<CareerGuideChatProps> = ({ onRecommendationComplete }) => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[650px] bg-neutral-50 dark:bg-neutral-900 rounded-[2rem] shadow-2xl overflow-hidden border border-white/20 relative">
      ...
      <div className="absolute top-0 left-0 w-full h-64 bg-gradient-to-b from-primary-100/60 to-transparent pointer-events-none dark:from-primary-900/20" />
      ...
      <div className="bg-white/80 dark:bg-neutral-800/80 backdrop-blur-md p-4 flex items-center border-b border-neutral-100 dark:border-neutral-700 z-10 sticky top-0">
        <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-primary-500 to-accent-400 flex items-center justify-center shadow-lg mr-4 ring-4 ring-white dark:ring-neutral-700">
            <SparkleIcon className="w-6 h-6 text-white animate-pulse-slow" />
        </div>
        <div>
            <h2 className="font-display font-bold text-lg text-neutral-900 dark:text-white leading-none">ShaktiPath Guide</h2>
            <p className="text-xs font-medium text-primary-600 dark:text-primary-400 flex items-center mt-1">
        ...
        {messages.map((msg, index) => {
            ...
            return (
              <div key={index} className={`flex w-full ${isModel ? 'justify-start' : 'justify-end'} animate-fade-in-up`}>
                {isModel && (
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex-shrink-0 mr-3 mt-auto mb-2 shadow-md flex items-center justify-center">
                        ...
                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
                            {options.map((opt, i) => (
                                <button
                                    ...
                                    className="group relative bg-white dark:bg-neutral-800 border-2 border-primary-100 dark:border-neutral-700 hover:border-primary-500 dark:hover:border-primary-500 p-3 rounded-xl text-left transition-all duration-200 shadow-sm hover:shadow-md active:scale-95"
                                >
                                    <div className="flex items-center">
                                        <div className="w-6 h-6 rounded-full bg-primary-50 dark:bg-neutral-700 text-primary-600 dark:text-primary-400 font-bold text-xs flex items-center justify-center mr-3 group-hover:bg-primary-600 group-hover:text-white transition-colors">
                                            {i + 1}
                                        </div>
                                        <span className="text-sm font-semibold text-neutral-700 dark:text-neutral-200 group-hover:text-primary-700 dark:group-hover:text-white">
                                            {opt}
                                        </span>
                                    </div>
                                </button>
        ...
        {error === "UNAUTHORIZED" && (
            <div className="flex justify-center my-4 animate-fade-in">
                <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 p-4 rounded-xl text-center">
                    <p className="text-sm text-red-600 dark:text-red-300 font-medium mb-3">Session expired. Please log in again.</p>
                    <button 
      ...
      {!isChatComplete && error !== "UNAUTHORIZED" && (
          <div className="p-4 bg-white dark:bg-neutral-800 border-t border-neutral-200 dark:border-neutral-700 z-10">
            <div className="flex space-x-3 items-end">
                <div className="flex-1 bg-neutral-100 dark:bg-neutral-700 rounded-2xl flex items-center px-4 border border-transparent focus-within:border-primary-300 focus-within:ring-2 focus-within:ring-primary-100 dark:focus-within:ring-neutral-600 transition-all">
                    <input
                        type="text"
                        value={input}
                ...
                <button
                    ...
                    className="bg-primary-600 text-white p-3 rounded-2xl hover:bg-primary-700 disabled:bg-neutral-300 dark:disabled:bg-neutral-600 transition-all shadow-lg shadow-primary-600/20 hover:shadow-primary-600/40 hover:scale-105 active:scale-95"
                >
