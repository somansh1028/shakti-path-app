...
const Feed: React.FC<FeedProps> = ({
    ...
    const toggleCommentSection = (postId: string) => {
        // If opening, ensure input is ready
        if (commentingPostId !== postId) {
            setCommentingPostId(postId);
        }
        
        // Toggle Visibility of previous comments
    ...
    const submitComment = async (postId: string) => {
        ...
        setCommentContent('');
        // Keep section open to see result
    ...
    return (
        <div className="space-y-5">
            {/* Create Post Box */}
            <div className="bg-white dark:bg-neutral-800 rounded-3xl p-4 shadow-soft">
                <p className="text-xs font-bold text-neutral-400 mb-2 uppercase tracking-wide">
                    {viewingCircle ? `Post to ${t(viewingCircle.nameKey)}` : 'Public Feed'}
                </p>
                <textarea
                    className="w-full bg-neutral-50 dark:bg-neutral-700 border-none rounded-xl p-3 text-sm focus:ring-2 focus:ring-primary-500 dark:text-white resize-none"
                    placeholder={viewingCircle ? "Share something with the group..." : "Share your learning journey..."}
                    rows={2}
                ...
                <div className="flex justify-end mt-2">
                    <button
                        ...
                        className="bg-primary-600 text-white text-xs font-bold py-2 px-4 rounded-full hover:bg-primary-700 disabled:bg-neutral-300 transition-colors"
                    >
            ...
            {isLoading ? (
                ...
                posts.map(post => {
                    ...
                    return (
                        <div key={post._id} className="bg-white dark:bg-neutral-800 rounded-3xl p-5 shadow-soft transition-all">
                            {/* Header */}
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-neutral-100 dark:border-neutral-700/50">
                                ...
                                {/* Comment Button Logic: Hide if it's MY post (per requirements) */}
                                {!post.isMine && (
                                    <button 
                                ...
                                {post.isMine && (
                                     <div className="flex items-center space-x-1 text-sm font-medium text-neutral-300 cursor-default">
                                         <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
                                         <span>{post.comments ? post.comments.length : 0}</span>
                                     </div>
                                )}
                            ...
                            {showComments && (
                                <div className="mt-4 pt-4 border-t border-neutral-100 dark:border-neutral-700">
                                    {/* Input - Only show if NOT mine */}
                                    {!post.isMine && (
                                        <div className="flex gap-2 mb-4">
                                            <input 
                                                type="text" 
                                                placeholder="Write a comment..." 
                                                className="flex-1 bg-neutral-50 rounded-full px-4 py-2 text-sm border-none focus:ring-1 focus:ring-primary-500"
